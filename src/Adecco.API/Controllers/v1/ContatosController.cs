...
) : ApiBaseController
{
    ...
    public async Task<IActionResult> AtualizarContato(
        int clienteId,
        [FromBody] ContatoRequestDto request
    )
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                500,
                $"Ocorreu um erro interno ao atualizar o contato do cliente: {ex.Message}"
            );
        }
    }
    ...
    public IActionResult RemoverContato(int clienteId, int contatoId)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                500,
                $"Ocorreu um erro interno ao remover o contato do cliente: {ex.Message}"
            );
        }
    }

    [HttpPost("/{clienteId}/contatos")]
    public async Task<IActionResult> IncluirContato(
        int clienteId,
        [FromBody] ContatoRequestDto request
    )
    {
